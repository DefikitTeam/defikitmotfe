...
const Leaderboard = () => {
  ...
  const [userWeeklyData, setUserWeeklyData] = useState<UserLeaderboardEntry[]>(
    []
  );
  const [poolWeeklyData, setPoolWeeklyData] = useState<PoolLeaderboardEntry[]>(
    []
  );
  ...
  const { getTrustPointDailyWalletTokenAction, trustPointDailyWalletToken } =
    useTrustPointDailyWalletToken();
  const { getTrustPointWeeklyWalletTokenAction, trustPointWeeklyWalletToken } =
    useTrustPointWeeklyWalletToken();
  const {
    ...
    if (trustPointDailyWalletToken.status === EActionStatus.Pending) {
      ...
      if (
        ...
        const processedPoolData = (
          ...
          trustScore: new BigNumber(item.trustScore).div(1e18).toFixed(7),
          volume: new BigNumber(item.volume).div(1e18).toFixed(7),
  ...
  useEffect(() => {
    if (trustPointWeeklyWalletToken.status === EActionStatus.Pending) {
      ...
      if (
        trustPointWeeklyWalletToken.status === EActionStatus.Succeeded &&
  ...
  useEffect(() => {
    if (activeTab !== 'monthly' || !chainConfig?.chainId || !selectedMonth) {
      return;
    }
  ...
  useEffect(() => {
    if (trustPointMonthlyWalletToken.status === EActionStatus.Pending) {
      ...
      if (
        trustPointMonthlyWalletToken.status === EActionStatus.Succeeded &&
        trustPointMonthlyWalletToken
      ) {
        const processedUserData = (
  ...
  const userColumns = [
    ...
    },
    {
      ...
      render: (address: string) => (
        ...
        >
          ...
          >
            {isMobile ? shortWalletAddress(address ? address : '') : address}
          </span>
        </motion.div>
      )
    },
    {
      ...
      sorter: (a: UserLeaderboardEntry, b: UserLeaderboardEntry) =>
        parseFloat(a.points || '0') > parseFloat(b.points || '0') ? 1 : -1,
    ...
    },
    {
      ...
      className: '!font-forza',
      sorter: (a: UserLeaderboardEntry, b: UserLeaderboardEntry) =>
        parseFloat(a.volume || '0') > parseFloat(b.volume || '0') ? 1 : -1,
  ...
  const poolColumns = [
    ...
    },
    {
      ...
      render: (poolId: string) => (
        ...
        >
          ...
          >
            {isMobile ? shortWalletAddress(poolId ? poolId : '') : poolId}
          </span>
        </motion.div>
      )
    },

    {
      ...
      sorter: (a: PoolLeaderboardEntry, b: PoolLeaderboardEntry) =>
        parseFloat(a.trustScore || '0') > parseFloat(b.trustScore || '0')
          ? 1
          : -1,
      render: (score: string) => (
        <motion.div
            ...
            >
              <div className="mb-4 flex justify-end">
                <DatePicker
                  ...
                  onChange={(date) => setSelectedDate(date || dayjs.utc())}
                  allowClear={false}
                  disabledDate={disabledDateGreaterThanCurrent}
                />
              </div>
                      ...
                      >
                        <Table
                          ...
                          onRow={(record) => ({
                            onClick: () =>
                              record.rank === 1 &&
                              celebrateTopRank(record.address),
                            className:
                              highlightedUser === record.address
            ...
            >
              <div className="mb-4 flex justify-end">
                <DatePicker
                  ...
                  value={selectedWeek}
                  className="!font-forza text-base"
                  onChange={(date) => setSelectedWeek(date || dayjs.utc())}
                  allowClear={false}
                  disabledDate={disabledDateGreaterThanCurrent}
                />
              </div>
            ...
            >
              <div className="mb-4 flex justify-end">
                <DatePicker
                  ...
                  value={selectedMonth}
                  className="!font-forza text-base"
                  onChange={(date) => setSelectedMonth(date || dayjs.utc())}
                  allowClear={false}
                  disabledDate={disabledDateGreaterThanCurrent}
                />
              </div>
