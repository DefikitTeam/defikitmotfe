...
const StyleCommunication = ({ mode }: { mode: string }) => {
  ...
  if (mode === 'launch-pool') {
                  ...
                  {data?.aiAgent?.style?.all &&
                    ...
                    data.aiAgent.style.all.map((x, index) => (
                      <CreateTypeChatItem
                        ...
                        title={t('ALL')}
                        content={x}
                        onChangeContent={onChange('all', index)}
                  ...
                  {data?.aiAgent?.style?.chat &&
                    ...
                    data.aiAgent.style.chat.map((x, index) => (
                      <CreateTypeChatItem
                        ...
                        title={t('CHAT')}
                        content={x}
                        onChangeContent={onChange('chat', index)}
                  ...
                  {data?.aiAgent?.style?.post &&
                    ...
                    data.aiAgent.style.post.map((x, index) => (
                      <CreateTypeChatItem
                        ...
                        title={t('POST')}
                        content={x}
                        onChangeContent={onChange('post', index)}
    ...
    const onDelete = (name: 'all' | 'chat' | 'post', index: number) => () => {
      setCreateAiAgentInformationAction({
        ...
        style: {
          ...dataAiAgent?.style,
          [name]: (dataAiAgent?.style?.[name] ?? []).filter(
            (r, i) => i != index
          )
        }
      });
    ...
    const onChange =
      (name: 'all' | 'chat' | 'post', index: number) => (value: string) => {
        const styleAlls = [...(dataAiAgent?.style?.[name] ?? [])];
        styleAlls[index] = value;
          ...
          <BoxArea>
            <div className="flex flex-col gap-2">
              <div className="">
                ...
                <div className="mb-6 flex flex-col gap-6">
                  {dataAiAgent?.style?.all &&
                    dataAiAgent.style.all.length > 0 &&
                    dataAiAgent.style.all.map((x, index) => (
                      <CreateTypeChatItem
                        key={index}
                        index={index + 1}
                        title={t('ALL')}
              ...
              <div>
                ...
                <div className="mb-6 flex flex-col gap-6">
                  {dataAiAgent?.style?.chat &&
                    dataAiAgent.style.chat.length > 0 &&
                    dataAiAgent.style.chat.map((x, index) => (
                      <CreateTypeChatItem
                        key={index}
                        index={index + 1}
                        title={t('CHAT')}
              ...
              <div>
                ...
                <div className="mb-6 flex flex-col gap-6">
                  {dataAiAgent?.style?.post &&
                    dataAiAgent.style.post.length > 0 &&
                    dataAiAgent.style.post.map((x, index) => (
                      <CreateTypeChatItem
                        key={index}
                        index={index + 1}
                        title={t('POST')}
