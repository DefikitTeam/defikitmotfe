...
>(
    ...
    async (param, { rejectWithValue }) => {
        try {
            const response =
                await servicePool.getTransactionByPoolAndSender(param);
...
const activitiSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(
                getAllTransactionByPoolAndSernder.pending,
                (state, action) => {
                    state.status = EActionStatus.Pending;
                }
            )
            .addCase(
                getAllTransactionByPoolAndSernder.fulfilled,
                (state, action) => {
            ...
            .addCase(
                getAllTransactionByPoolAndSernder.rejected,
                (state, action) => {
