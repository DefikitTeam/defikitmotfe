...
>('pool/getDetailPoolBackground', async (params, { rejectWithValue }) => {
  try {
    ...
    const [
      ...
      servicePool.getDetailPoolDataFromServer(chainId.toString(), poolAddress),
...
>('pool/getHolderDistribution', async (params, { rejectWithValue }) => {
  try {
    const holderDistribution = await servicePool.getHolderDistribution(params);
...
export const getAllRankPools = createAsyncThunk<
  IGetAllRankPoolsResponse,
  IGetAllRankPoolsParams,
  {
    rejectValue: FetchError;
  }
>('pool/getAllRankPools', async (params, { rejectWithValue }) => {
  try {
    ...
    let totalPool = 0;
    while (true) {
      const rankPools = await servicePool.getAllRankPools({
        skip: skipReal,
        chainId
      });
...
export const poolDetailSlice = createSlice({
  ...
  reducers: {
    ...
    setPageHolderDistribution: (
      ...
      state.pageHolderDistribution = action.payload.isPageHolderDistribution;
    },
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<IDetailPoolBackgroundResponseData>) => {
          state.status = EActionStatus.Succeeded;
          state.dataDetailPoolFromServer =
            action.payload.dataDetailPoolFromServer;
          state.transactions = action.payload.transactions;
    ...
    builder
      ...
      .addCase(getAllRankPools.fulfilled, (state, action) => {
