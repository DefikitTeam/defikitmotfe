...
import { IGetTrustScoreHistoryWalletParams, ITrustScoreHistoryWalletState, IWalletTrustScoreHistory } from "./type";
...
>('wallet/getTrustScoreHistoryWallet', async (params, { rejectWithValue }) => {
    ...
    try{
        ...
        console.log('listHistoryWallet line 34-----', listHistoryWallet);
...
export const trustScoreHistoryWalletSlice = createSlice({
    ...
    reducers: {
        ...
        setOpenModalHistoryWallet: (state, action: PayloadAction<{ isOpenModalHistoryWallet: boolean }>) => {
            state.openModalHistoryWallet = action.payload.isOpenModalHistoryWallet;
        }
    },
    extraReducers: (builder) => {
        builder.addCase(getTrustScoreHistoryWallet.pending, (state) => {
...
export const { resetTrustScoreHistoryWallet, setOpenModalHistoryWallet } = trustScoreHistoryWalletSlice.actions;
