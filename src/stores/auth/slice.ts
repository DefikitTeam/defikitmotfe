...
>('auth/loginWallet', async (loginData, { rejectWithValue }) => {
  try {
    ...
    const { user, tele, wallet, accessToken, refreshToken, twitter, discord } =
      loginResponse;
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    updateSignature: (state: IAuthState, action: PayloadAction<ISignature>) => {
    ...
    setOpenModalInviteBlocker: (
      ...
      state.openModalInviteBlocker = action.payload.isOpenModalInviteBlocker;
    }
  },
