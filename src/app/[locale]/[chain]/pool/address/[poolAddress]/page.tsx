...
const safeExtractMetadata = (tokenData: any, metadataResult: any) => {
  ...
  try {
    const tokenName =
      tokenData?.data?.name || tokenData?.name || fallback.title;
    const tokenDescription =
      metadataResult?.metadata?.description || fallback.description;
    ...
    return {
      title:
        typeof tokenName === 'string' && tokenName.length > 0
          ? tokenName
          : fallback.title,
      description:
        typeof tokenDescription === 'string' && tokenDescription.length > 0
          ? tokenDescription
...
export async function generateMetadata(
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
    ...
    try {
      const { title, description, image } = safeExtractMetadata(
        cached.data.tokenData,
        cached.data.metadataResult
      );
