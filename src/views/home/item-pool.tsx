  ...
  useEffect(() => {
    ...
    const sumEth = new BigNumber(
      poolItem.buyTransactions
        ...
        .reduce((total, transaction) => total + Number(transaction.eth), 0)
    ).div(1e18);
    ...
    >
      ...
      >
        ...
        <div className="ml-2 flex w-80 flex-col text-left !font-forza">
          ...
          >
            Farming APY:{' '}
            <span className="text-lg font-extrabold text-green">{apyShow}</span>
          </Text>
          <Text
            className={`!font-forza ${isMobile ? '' : 'text-lg-important'}`}
          >
            ...
            <span className="text-blue-800">
              {Number(poolItem.latestTimestampBuy) === 0 ? (
                ...
                <Moment fromNow>
                  {new Date(Number(poolItem.latestTimestampBuy) * 1000)}
                </Moment>
              )}
            </span>
          </Text>
