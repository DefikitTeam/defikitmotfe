...
const CreateLaunch = () => {
  ...
  const onFinish = async () => {
    ...
    try {
      if (avatarInfo?.flag) {
        const res = await serviceUpload.getPresignedUrlAvatarWithoutAddress(
      ...
      const metadata = {
        ...
        website:
          data.websiteLink && !data.websiteLink.startsWith('https://')
            ...
            : data.websiteLink,
        telegram:
          data.telegramLink && !data.telegramLink.startsWith('https://')
      ...
      const resData = await serviceUpload.uploadMetadataToServerWithoutAddress(
        metadataPayload,
        chainConfig?.chainId.toString()!,
        signature,
        address as `0x${string}`
      ...
      const maxDurationSell =
        new Date(data.endTime).valueOf() - new Date(data.startTime).valueOf();
        ...
        >
          ...
          >
            ...
            <SaveCreatePoolButton
              ...
              disiabled={isLoadingCreateLaunch || !address || !avatarInfo?.flag}
            />
          </Form>
        </Col>
      </div>
