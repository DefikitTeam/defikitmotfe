...
const SellToken = () => {
  ...
  const [defaultTokenAddress, setDefaultTokenAddress] = useState<KeyValueObj>();
  ...
  useEffect(() => {
    ...
    if ((address as `0x${string}`) && portfolio.investedPools.length > 0) {
      let activeInvestPools = portfolio.investedPools.filter((item: IPool) => {
        return (
  ...
  useEffect(() => {
    if ((address as `0x${string}`) && portfolio.idChooseTokenSell) {
      const token = portfolio.investedPools
        ...
        .find(
          (item) =>
            item.id.toLowerCase() === portfolio.idChooseTokenSell?.toLowerCase()
        );
      if (token) {
    ...
    if (selectedToken) {
      if (parseInt(value) > parseInt(balanceOfUser)) {
        ...
        if (value) {
          if (
            selectedToken &&
            parseInt(selectedToken.startTime) * 1000 <= now.valueOf()
          ) {
            setDisableBtnSell(false);
          } else {
            setDisableBtnSell(true);
        ...
        <Row gutter={[0, 0]}>
          ...
          >
            <div className="mb-0 text-lg">
              <span className="!font-forza text-lg">{t('TOKEN_ADDRESS')}</span>
              <Select
                placeholder={
                ...
                options={tokenAddressList.map((item) => ({
                  value: item.key,
                  label: (
                    <span className="!font-forza text-base">{item.value}</span>
                  )
                }))}
              />
            </div>
          ...
          >
            <div className="mb-0">
              <div className="font-forza text-lg ">{t('AMOUNT_PER_BOND')}</div>
          ...
          >
            <Slider
              ...
              min={0}
              max={
                parseInt(balanceOfUser) > 100 ? 100 : parseInt(balanceOfUser)
              }
              marks={parseInt(balanceOfUser) >= 100 ? markSlider : {}}
              onChange={handleChangeSlider}
              value={typeof sliderPercent === 'number' ? sliderPercent : 0}
            />
          </Col>
          <Col
            xs={24}
